...
public class BusCheckServerSyncforLastDate extends AsyncTask<Void, Void, String>{
Activity mActivity;
Context context;
...
public BusCheckServerSyncforLastDate(Activity otherActivity, ProgressDialog progressDialog2,
                                     Context applicationContext, int fleetNumber,
                                     String SectionList,
                                     String AnswerList,
                                     String Date, String android_id, String lat, String lag, String TestDrive, String GeneralNotes, int i) {
...
protected void onPostExecute(String result) {
    if(result.equals("0")){
       ...
       final MySQLiteHelper my = new MySQLiteHelper(this.mActivity);
        my.updateinsertINTOfleetCheckList(((MyApplication) this.mActivity.getApplication()).BusId, this.lag, this.lat, 1, this.TestDrive, this.GeneralNotes, ((MyApplication) this.mActivity.getApplication()).GoodSeatsCount, ((MyApplication) this.mActivity.getApplication()).BadSeatsCount, ((MyApplication) this.mActivity.getApplication()).NASeatsCounts, ((MyApplication) this.mActivity.getApplication()).KMCounter, ((MyApplication) this.mActivity.getApplication()).SeatBeltsCount, ((MyApplication) this.mActivity.getApplication()).NASeatBeltsCount, ((MyApplication) this.mActivity.getApplication()).BatteryVoltage, this.Date);
        my.update(((MyApplication) this.mActivity.getApplication()).AnswerAndQuestion, prefs.getString("BusId", "0") , prefs.getString("Date", "0"));
        my.updateStatus(1, this.Date);


        this.mActivity.getSharedPreferences("Date", 0).edit().clear().apply();
        this.mActivity.getSharedPreferences("BusId", 0).edit().clear().apply();
        this.mActivity.getSharedPreferences("fleetnumber", 0).edit().clear().apply();
        ((MyApplication) this.mActivity.getApplication()).AnswerAndQuestion.clear();
        //((MyApplication) this.mActivity.getApplication()).BusId;
